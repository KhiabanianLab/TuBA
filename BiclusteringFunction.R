BiclusteringFunction <- function(FileNameGenePairs,FileNameBinaryMatrix,OverlapCutOff)
{
  ...
  while (i < length(List.of.Genes.In.Clusters))
  {
    ...
    if (length(x2$Node1) > 0)
    {
      ...
      Temp.List.of.Seeds <- as.numeric(V(g)$name[unlist(List.of.Largest.Cliques[[1]])])
      
      Temp.Largest.Clique.Length <- length(Temp.List.of.Seeds)
      
      if (Temp.Largest.Clique.Length >= 3)
      {
         List.of.Seeds[[t]] <- Temp.List.of.Seeds
      }
    }
  }
